class WordCloudData {
    ...
    populateWordsToCounts(inputString) {
      ...
      let inputAr = inputString.split(' ')
      inputAr = inputAr.map(e => e.split('').filter(e2 => e2 !== '.' && e2 !=='!' && e2 !==',' && e2 !== '?' && e2 !== '-' && e2 !== 
      "'").join('').toLowerCase()).filter(e => e !=='')
      console.log(inputAr)
      inputAr.forEach(e => {
        if (this.wordsToCounts.has(e)) {
          this.wordsToCounts.set(e, this.wordsToCounts.get(e)+1)
        }
        else {
          this.wordsToCounts.set(e, 1)
        }
      })
