...
function colorGraph(graph, colors) {
  ...
  const maxColors = graph.reduce( (largestDegree=0, node ) => node.neighbors.size > largestDegree ? node.neighbors.size : largestDegree)
  
  for (const node in graph ) {
    availColors = colors.slice(0, maxColors)
